...
"""
    1. ธนาคารให้บริการ User เปิดบัญชี โดยมีบัญชี 2 ประเภท คือ 1) ออมทรัพย์ 2) ฝากประจํา ซึ่งมีอัตราดอกเบี้ยไม่เท่ากัน โดยออมทรัพย์จะได้ 0.5 และฝากประจําจะได้ 2.5
    2. User 1 คนจะเปิดกี่บัญชีก็ได้ 
       โดยข้อมูลของ User ประกอบด้วย 
            1) เลขประจําตัวประชาชน (สมมติ) 2) ชื่อ-นามสกุล 
       ส่วนข้อมูลของบัญชีประกอบด้วย 
            1) หมายเลขบัญชี 2) user เจ้าของบัญชี
    3. ในบัญชีประเภทออมทรัพย์ สามารถขอบัตร ATM หรือบัตร Debit ได้ 1 บัตร
        • บัตร ATM และบัตร Debit ใช้กับเครื่อง ATM ได้ โดยบัตร ATM จะมีหมายเลขบัตร และข้อมูลในบัตร ได้แก่ 1) หมายเลขบัญชี 2) PIN Number
        • บัตร Debit จะมีคุณสมบัติเหมือนกับบัตร ATM และสามารถซื้อของในร้านที่รับบัตร Debit ได้ด้วย
    4. ธนาคารจะมีหลักเกณฑ์และอัตราต่าง ๆ ได้แก่ 1) บัตร ATM จะมีค่าธรรมเนียมรายปี 150 บาท แต่ถ้าเป็นบัตร Debit จะมีค่าธรรมเนียมรายปี 300 บาท
    5. ผู้ใช้บัตรทั้ง 2 แบบสามารถใช้บัตรกับเครื่อง ATM 
       โดยตู้ ATM จะมีข้อมูลคือ 1) หมายเลขตู้ 2) เงินที่มีในตู้ กําหนดให้ค่าเริ่มต้นคือ 1,000,000 บาท 
       ตู้ ATM จะมีข้อจํากัดในการถอน โดยถอนได้ไม่เกิน 20,000บาทต่อการกด 1 ครั้ง
    6. ในการใช้ Debit สามารถใช้กับเครื่องรูดบัตร (EDC) ได้ หรือใช้วิธีอิเล็กทรอนิกส์ก็ได้ 
       โดยเครื่องรูดบัตรจะมีข้อมูล คือ 1) หมายเลขเครื่อง 2) ชื่อผู้ขายหรือผู้ขอตัดบัญชี 
       ถ้าเป็นวิธีอิเล็กทรอนิกส์ก็จะมีผู้ขอตัดบัญชี กําหนดให้สร้าง class ของผู้ขาย (สามารถรับชําระเงินทางอิเล็กทรอนิกส์) และระบุ EDC ของผู้ขายแต่ละราย (สามารถรูดซื้อสินค้าได้)
    7. การทํากิจกรรมกับธนาคารมี 3 ช่องทาง
        • ผ่านเครื่อง ATM ประกอบด้วย 1) การฝากเงิน (Deposit) 2) การถอนเงิน (Withdraw) และการโอนเงิน (Transfer) 
            โดยการถอนเงิน จะต้องไม่เกิน อัตราเบิกเงินสูงสุดต่อครั้งและต้องไม่เกินเงินในตู้ ATM และ การโอนเงินต้องไม่เกิน 100,000 บาท
        • ผ่านเครื่องรูดบัตร หรือ วิธีอิเล็กทรอนิกส์ซึ่งเป็นการโอนเงินจากบัญชีของผู้จ่ายไปยังบัญชีของผู้ขาย กรณีผ่านเครื่องรูดบัตร
        • ผ่านหน้า Counter ประกอบด้วย การฝากเงิน และ การถอนเงิน
    8. ในการทํารายการแต่ละครั้งต้องมีการบันทึกได้แก่ 1) ประเภทรายการ (D=Deposit, W=Withdraw, T=Transfer) 2) จํานวนเงิน 3) จํานวนเงินคงเหลือ 4) กรณีโอนจะเก็บเลขบัญชีที่โอนไปด้วย
"""
...
class Bank :
    ...
    def add_atm_machine(self, atm_machine) :
        if not isinstance(atm_machine, ATM_machine) :
            return False
...
class Account:
    ...
    def transfer(self, account, amount, target_account) :
        if not isinstance(target_account, Account) or amount > 100000 :
            return False
        self.__rshift__(amount, target_account)

        transact = Transaction("T", amount, self.amount, target_account.account_no)
        self.__transaction.append(transact)
    ...
    def __rshift__(self, amount, target_account) :
        if not isinstance(target_account, Account) or not isinstance(amount, (int, float)) :
            return False
        self.__amount -= amount
...
class Transaction :
    def __init__(self, _type, amount, total, target_account) :
        self.__type = _type
        self.__amount = amount
        self.__total = total
    ...
    def __str__(self) :
        return f"{self.__type}-{self.__amount}-{self.__total}-{self.__target_account}"
...
class ATM_machine :
    ...
    def deposit(self, account, amount) :
        ...
        account + amount
        transact = Transaction("D", amount, account.amount, None)
        account.transaction = transact


    def withdraw(self, account, amount) :
        if not isinstance(account, Account) or (amount > account.amount) or amount <= 0 or amount > ATM_machine.withdraw_limit or amount > self.__money:
            return False
        self.__money -= amount
        # account.__sub__(amount)
        account - amount
        transact = Transaction("W", amount, account.amount, None)
        account.transaction = transact

    # def transfer(self, counter, amount, target_account) :
    #     if not isinstance(target_account, Account) or amount <= 0 :
    #         return False
    #     self >> target_account
...
class Seller :
    ...
    def paid(self, account, amount, seller_acc) :
        if not isinstance(account, Account) or not isinstance(seller_acc, Account) :
            return False
        account - amount
        seller_acc + amount
        transact = Transaction("T", amount, account.amount, seller_acc.account_no)
        account.transaction = transact
...
class EDC_machine :
    ...
    def paid(self, card, amount, seller_acc) :
        if not isinstance(card, Card) or amount <= 0 :
            return False
        seller_acc.amount += amount
        card.account.amount -= amount
        transact = Transaction("T", amount, card.account.amount, seller_acc.account_no)
        # card.add_Transactions(transact)
        card.account.transaction = transact
    @property 
    def edc_no(self) :
...
for transact in hermione_account.transaction :
    print(transact)